...
namespace EzySlice {
	...
	public sealed class IntersectionResult {
		...
		public void OnDebugDraw() {
			OnDebugDraw(UnityEngine.Color.white);
		}

		public void OnDebugDraw(UnityEngine.Color drawColor) {
            #if UNITY_EDITOR
			...
			UnityEngine.Color prevColor = Gizmos.color;
			...
			for (int i = 0; i < upperHullCount; i++) {
				upperHull[i].OnDebugDraw(UnityEngine.Color.red);
			}

			// draw the lower hull in BLUE
			for (int i = 0; i < lowerHullCount; i++) {
				lowerHull[i].OnDebugDraw(UnityEngine.Color.blue);
			}
